...
export default function Card(props: CardProps) {
  ...
  return (
    <div className="group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
      <div className="absolute -inset-x-4 -inset-y-4 z-0 hidden rounded-md transition motion-reduce:transition-none lg:-inset-x-6 lg:block lg:group-hover:bg-[var(--sub-alt-color)]"></div>
      ...
      >
        <h3 className="font-medium leading-snug">
          <div>
            <Link
              className="inline-flex items-baseline font-medium leading-tight text-theme-text hover:text-theme-main group/link text-base"
              href={props.link}
              target="_blank"
              rel="noreferrer"
              aria-label={
      ...
      {isProject && props.image && (
        <Image
          ...
          height={120}
          decoding="async"
          className="rounded transition sm:order-1 sm:col-span-2 sm:translate-y-1"
